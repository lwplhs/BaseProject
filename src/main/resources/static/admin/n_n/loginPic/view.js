...
function loginPic_save_submit(){
    var jsondata = $("#form-loginPic-add").serialize();
    console.log(jsondata);
    $.ajax({
        type:"POST",
        data:jsondata,
        url:"/admin/saveLoginPic",
...
/*                text = '共' + fileCount + '张（' +
        ...
        uploader.on("beforeFileQueued",function () {
            var _UploadFile = uploader.getFiles();
            if (_UploadFile.length > 0){
                uploader.removeFile(_UploadFile[_UploadFile.length - 1], true);
            }
        });
